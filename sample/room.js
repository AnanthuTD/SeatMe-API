...
function generateDummyRoomData() {
    ...
    for (let i = 1; i <= numberOfEntries; i += 1) {
        const id = i;
        const internalCols = Math.floor(Math.random() * (6 - 4 + 1)) + 4; // Random number of columns (4 to 6)
        const finalCols = Math.floor(Math.random() * (6 - 4 + 1)) + 4; // Random number of columns (4 to 6)
        const internalRows = Math.floor(Math.random() * (7 - 4 + 1)) + 4; // Random number of rows (4 to 7)
        const finalRows = Math.floor(Math.random() * (7 - 4 + 1)) + 4; // Random number of rows (4 to 7)
...
models.room.bulkCreate(roomData, { validate: true });
